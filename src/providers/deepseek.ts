...
export class DeepseekProvider extends BaseLLMProvider {
  ...
  constructor() {
    const configured = !!config.deepseek.apiKey;
    super('deepseek', config.deepseek.model, configured);
    if (configured) {
  ...
  async sendMessage(messages: ChatMessage[]): Promise<string> {
    ...
    if (this.isOnCooldown()) throw new Error('Deepseek provider is on cooldown');
